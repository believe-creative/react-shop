...
app.post('/setpassword', [
  ...
  if(req.session.user)
  {
    ...
    if (!errors.isEmpty()) {
      console.log(errors.isEmpty(),errors.array(),"came here post erors---------------------");
    ...
    else
    {
      cryptPassword(req.body.pwd,function(error,hash){
          if(hash)
          {
            sequelize.query('CALL customer_add(:inName,:inEmail,:inPassword)',
            {replacements: {inName:req.body.name,inEmail:req.body.email,inPassword:hash}}).then(function(){ return res.redirect('loginsuccess');});
          }
...
app.post('/api/get-attribute-values', checkToken, (req,res)=>{
  ...
  sequelize
    .query('CALL catalog_get_attribute_values(:inAttributeId)',{replacements:{inAttributeId:inAttributeId}}).then(
      get_attribute_values=>res.json(get_attribute_values));
});
...
app.post('/api/get-product-recommendations', checkToken, (req,res)=>{
  ...
  let inShortProductDescriptionLength=req.body.inShortProductDescriptionLength;
  sequelize
    .query('CALL catalog_get_recommendations(:inProductId,:inShortProductDescriptionLength)',{replacements:{inProductId:inProductId,inShortProductDescriptionLength:inShortProductDescriptionLength}}).then(
      get_recommendations=>res.json(get_recommendations));
});
/*
...
app.post('/api/get-customer', checkToken, (req,res)=>{
  ...
  else
  {
    ...
    {replacements:{inEmail:inEmail}}).then(
      customer_info=>{
        sequelize
    .query('CALL customer_get_customer(:inCustomerId)',{replacements:{inCustomerId:customer_info[0].customer_id}}).then(
      get_customer=>res.json(get_customer));
...
app.post('/api/check-user', checkToken, (req,res)=>{
  ...
  sequelize
    .query('CALL customer_get_login_info(:inEmail)',{replacements:{inEmail:inEmail}}).then(
      get_login_info=>res.json(get_login_info));
});
...
app.post('/api/update-account', checkToken, (req,res)=>{
  ...
  sequelize
    .query('CALL customer_update_account(:inCustomerId,:inName,:inEmail,:inPassword,:inDayPhone,:inEvePhone,:inMobPhone)',
    {replacements:{inCustomerId:inCustomerId,inName:inName,inEmail:inEmail,inPassword:inPassword,inDayPhone:inDayPhone,inEvePhone:inEvePhone,inMobPhone:inMobPhone}}).then(
...
app.post('/api/add-address',checkToken, jsonParser, (req,res)=>{
  ...
  sequelize
    ...
    {replacements:{inEmail:inEmail}}).then(
      customer_info=>{
        sequelize
        .query('CALL customer_add_address(:inCustomerId,:inAddressName,:inAddress1,:inAddress2,
          :inCity,:inPostalCode,:inCountry,:inDayPhone,:inEvePhone,:inMobPhone)',
        {
          replacements:{
          ...
          add_address=>res.json(add_address));
      });
...
app.post('/api/update-address',checkToken, jsonParser, (req,res)=>{
  ...
  sequelize
    ...
    {replacements:{inEmail:inEmail}}).then(
      customer_info=>{
        sequelize
        .query('CALL customer_update_address(:inCustomerId,:inAddressName,:inAddress1,:inAddress2,
          :inCity,:inPostalCode,:inCountry,:inDayPhone,:inEvePhone,:inMobPhone)',
        {
          replacements:{
            inCustomerId:customer_info[0].customer_id,
...
app.post('/api/update-credit-card', checkToken, (req,res)=>{
  ...
  sequelize
    ...
    {replacements:{inCustomerId:inCustomerId,inCreditCard:inCreditCard}}).then(
      update_credit_card=>res.json(update_credit_card));
});
...
app.post('/api/get-cart-total', checkToken, (req,res)=>{
  ...
  sequelize
    ...
    {replacements:{inCartId:inCartId}}).then(
      get_total_amount=>res.json(get_total_amount));
});
